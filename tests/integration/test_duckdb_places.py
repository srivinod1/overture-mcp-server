...
class TestPlacesInRadius:
    ...
    async def test_excludes_permanently_closed_by_default(self, test_registry):
        ...
        assert "Mokum Coffee" not in names
    ...
    async def test_includes_temporarily_closed_by_default(self, test_registry):
        ...
        names = [r["name"] for r in result["results"]]
        assert "Canal Coffee" in names

    async def test_include_closed_returns_all(
        self, test_registry, known_coffee_shops_within_500m_including_closed,
    ):
        ...
        assert result["count"] == known_coffee_shops_within_500m_including_closed
        names = [r["name"] for r in result["results"]]
        assert "Mokum Coffee" in names
...
class TestPlacesBrandAndAddress:
    ...
    async def test_unbranded_place_has_null_brand(self, test_registry):
        """Non-chain coffee shops should have null brand."""
        result = await execute_operation(test_registry, "places_in_radius", {
            "lat": 52.3676, "lng": 4.9041, "radius_m": 500, "category": "coffee_shop",
        })
        koffie = [r for r in result["results"] if r["name"] == "Koffie Centrum"]
        assert len(koffie) == 1
    ...
    async def test_place_with_freeform_address(self, test_registry):
        ...
        # Koffie Centrum has freeform address "Damrak 1, 1012 LG Amsterdam"
        koffie = [r for r in result["results"] if r["name"] == "Koffie Centrum"]
        assert len(koffie) == 1
    ...
    async def test_place_with_website(self, test_registry):
        """First coffee shop should have a website."""
        result = await execute_operation(test_registry, "places_in_radius", {
            "lat": 52.3676, "lng": 4.9041, "radius_m": 500, "category": "coffee_shop",
        })
        # Koffie Centrum (idx 0) has a website
        koffie = [r for r in result["results"] if r["name"] == "Koffie Centrum"]
        assert len(koffie) == 1
    ...
    async def test_place_with_phone(self, test_registry):
        """First coffee shop should have a phone."""
        result = await execute_operation(test_registry, "places_in_radius", {
            "lat": 52.3676, "lng": 4.9041, "radius_m": 500, "category": "coffee_shop",
        })
        koffie = [r for r in result["results"] if r["name"] == "Koffie Centrum"]
...
class TestNearestPlace:
    ...
    async def test_nearest_excludes_permanently_closed(self, test_registry):
        ...
        assert result["results"][0]["name"] != "Mokum Coffee"
...
class TestCountPlaces:
    ...
    async def test_count_with_include_closed(
        ...
        assert count_result["results"][0]["count"] == known_coffee_shops_within_500m_including_closed
